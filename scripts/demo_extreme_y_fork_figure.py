# -*- coding: utf-8 -*-
"""Extreme neuron bifurcation schematic: convex-hull vs Y-Fork (quad pipes).

Schematic geometry matches the first figure style from `_draw_extreme.py`
(colors, camera, extreme thin-C / thick-parent layout), but branch extensions
are unsubdivided 4-sided quad pipes (not circular tubes), junction strips are
index-aligned to avoid twist, and the PNG has no on-image text.
"""
...
# ---------------------------------------------------------------------------
# Geometry helpers (from _draw_extreme.py + quad-pipe / alignment)
# ---------------------------------------------------------------------------
...
def best_shift(Ra, Rb, allow_reverse=True):
    ...
    candidates = [(False, Rb)]
    if allow_reverse:
        # reverse orientation while keeping vertex 0 as a start candidate;
        # all cyclic starts are still searched via roll below
        Rb_rev = Rb[::-1]
...
# ---------------------------------------------------------------------------
# Extreme schematic geometry (same constants as _draw_extreme.py)
# ---------------------------------------------------------------------------
...
def build_geometry():
    ...
    dir_B = normalize([0.18, 0.40, 0.90])
    dir_C = normalize([-0.96, 0.08, 0.22])  # sharp turn

    A_c = fork + parent_dir * 0.55
    B_c = fork + dir_B * 0.72
    C_c = fork + dir_C * 0.58
    P_end = fork + parent_dir * 2.25
    B_end = fork + dir_B * 2.45
    C_end = fork + dir_C * 2.05
    rP, rB, rC = 0.40, 0.27, 0.085

    # Junction connection rings (slightly inset / scaled like the original)
    # Share a common phase basis on A, then align B and C to remove twist.
    phase_A = 0.5  # matches original ring4 default spirit
    A_L = ring4(A_c - parent_dir * 0.02, -parent_dir, rP * 1.02, phase=phase_A)
    B_L_raw = ring4(B_c - dir_B * 0.06, dir_B, rB * 1.03, phase=0.0)
    ...
    j = (skip + 1) % 4
    # Opening quad winding matching strip_faces vertex pairing:
    # edge skip on A/B is left open; order A[s], A[j], B[j], B[s]
    ...
    # Display rings at pipe / hull boundary (aligned consistently with junction)
    ring_A = ring4(A_c, -parent_dir, rP, phase=phase_A)
    # Match B/C display rings to aligned junction rings via phase search
    ...
    # Re-align display rings exactly (phase search is approx; shift is exact)
    ...
    # Pipe phases so end/start rings match junction rings
    ph_P = best_phase_for_target(P_end, parent_dir, rP * 0.95, ring_A)
    # Parent pipe travels P_end -> A_c along parent_dir; tangent for ring4 is
    # (A_c - P_end) which is -parent_dir * ..., but frame(-t) flips b — use the
    # same tangent as ring_A for consistency: -parent_dir when building at A.
    # quad_pipe uses (p1-p0); for P_end->A_c, p1-p0 = A_c-P_end || -parent_dir? 
    # A_c = fork + parent_dir*0.55, P_end = fork + parent_dir*2.25
    # A_c - P_end = parent_dir*(0.55-2.25) = -parent_dir * 1.7, so tangent = -parent_dir
    # which matches ring_A's tangent. Good.
...
def _copy_outputs(src: Path) -> list[Path]:
    ...
    targets = [
        ...
        Path(r"D:\QuadMeshTesser\Paper\figs_223") / src.name,
        Path(r"C:\Users\15370") / src.name,
    ]
    # Also ensure docs path is written (src may already be there)
    seen: set[str] = set()
    for dst in targets:
        try:
            key = str(dst.resolve())
...
def main() -> int:
    ...
    png = out_dir / "fig_extreme_bifurcation_compare.png"
    render_compare(g, png, elev=20, azim=-58, dpi=230)
    copied = _copy_outputs(png)

    png_alt = out_dir / "fig_extreme_bifurcation_compare_alt.png"
